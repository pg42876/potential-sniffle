...
class Dataset:
    ...
    def from_data(cls, filename, sep = ",", labeled = True):
        ...
        # Numpy genfromtxt() to load the data from the text files, with missing values handled as specified.
        ...
        if labeled: # vê se tem uma label
            X = data[:, 0:-1]
            y = data[:, -1]
        else:
            X = data
    ...
    def from_dataframe(cls, df, ylabel = None):
        ...
        if ylabel and ylabel in df.columns: # ver se tem a label dependente
            ...
            yname = ylabel
        else: # caso não tenha label
            X = df.to_numpy() # converte diretamente para numpy array
            y = None # não tem label
            xnames = list(df.columns) # nomes das variáveis independentes
            yname = None
    ...
    def __len__(self):
        ...
        return self.X.shape[0] # retorna o número de linhas
    ...
    def getNumFeatures(self):
        ...
        return self.X.shape[1] # retorna o número de colunas
